...
void main(){
  ...
  float base = fbm2(uv, uTime * 0.06);
  // Autonomous ambient wave field so the pattern is vibrant and animated continuously without mouse clicks
  float ambient = base * 0.7 - 0.15 + (uDensity - 1.0) * 0.25;
  float wave1 = sin(length(uv * 2.2) - uTime * 0.5) * 0.18;
  float wave2 = cos(uv.x * 2.0 + uv.y * 1.5 - uTime * 0.35) * 0.12;
  float feed = ambient + wave1 + wave2;
...
export const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Clamp pixel ratio to max 1.5 to guarantee high 60fps performance across devices
      ...
      // Auto pause offscreen with IntersectionObserver for maximum performance
      ...
      if (noiseAmount > 0) {
        ...
        if (composer && composer.passes.length > 0) {
          composer.passes.forEach((p) => {
            const pass = p as unknown as PassWithEffects;
      ...
      // Only attach pointer listeners if ripples or liquid interaction are enabled
      if (enableRipples) {
